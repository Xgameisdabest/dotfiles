...
$TERMINAL -e bash -c "git pull || { clear; read -p 'Update failed, unable to perform git pull! Press <ENTER> to exit...'; exit 0; }; read -p 'Update completed. Press <ENTER> to continue and restart...'; stow -D .;stow .;sudo updatedb; hyprctl reload; clear; read -p 'Press <ENTER> to continue to see the changes, \"󰖳 + x\" to quit!';git diff HEAD@{1}"
